...
class CapsuleNetTester(unittest.TestCase):
    def get_random_inputs(self):
        ...
        y = np.random.randint(low=0, high=self.n_caps[-1], size=(self.batch_size,))
    ...
    def setUp(self):
        ...
        self.capsule_net = CapsuleNetwork(batch_size=self.batch_size, capsule_dims=self.capsule_dims, n_caps=self.n_caps)
    ...
    def test_ouput_prediction_shape_for_test_input(self):
        ...
        self.assertEqual(capsules.shape, (self.batch_size, self.n_caps[-1], self.capsule_dims[-1]))

    def test_capsule_transformation_matrices(self):
        weight_matrices = self.capsule_net.transformation_weights.detach().squeeze().cpu().numpy()
        self.assertEqual(weight_matrices.shape, (self.n_caps[0], self.n_caps[1], self.capsule_dims[1], self.capsule_dims[0]))
